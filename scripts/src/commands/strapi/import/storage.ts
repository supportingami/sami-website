...
export class StorageImport {
  public async run(parsedEnv?: any) {
    const { GCS_PUBLIC_BUCKET_NAME, GOOGLE_APPLICATION_CREDENTIALS } = parsedEnv;
    if (GCS_PUBLIC_BUCKET_NAME) {
      ...
      // E.g. Script
      // gcloud storage rsync -r data/public gs://sami_website_public
      ...
      return this.importGoogleStorage(GCS_PUBLIC_BUCKET_NAME);
    }
